...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    DEPENDENCIES=("git" "gcc" "make" "vim")

    for DEP in "${DEPENDENCIES[@]}"
    do
        if ! hash "$DEP" 2>/dev/null; then
            echo "Installation de $DEP..."
            sudo apt-get install -y "$DEP" || { echo "Erreur lors de l'installation de $DEP"; exit 1; }
        fi
    done
else
    echo "OS non pris en charge. Ce script ne fonctionne que sous Linux basé sur Debian."
    exit 1
fi

# Ajout du fichier .vimrc
cat << EOF > ~/.vimrc
